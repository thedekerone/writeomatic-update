...
function chatbot_updatePrompts() {
	$( '#chat_images' ).empty();
...
$(document).on("click", "#chatbot-trigger", function (e) {
	...
	if ( wrapper.hasClass('opacity-100') ){
		wrapper.removeClass('opacity-100 visible');
		wrapper.addClass('opacity-0 invisible');
	} else {
		wrapper.addClass('opacity-100 visible');
		wrapper.removeClass('opacity-0 invisible');
		if ( !chatbot ){
			startNewChatBot();
		} else {
