...
{   
    ...
    public static function getSubscriptionStatus($fromApi=false) {
        ...
        if($gateway == null){
            Log::error('Gateway not found. RevenueCatController@getSubscriptionStatus');
            if($fromApi){
        ...
        if($apiKey == null){
            Log::error('Gateway is not set properly. RevenueCatController@getSubscriptionStatus');
            if($fromApi){
        ...
        if($userRevenueCatId == null){
            Log::error('User is not set properly. User must login at least once from mobile app. RevenueCatController@getSubscriptionStatus');
            if($fromApi){
        ...
        if($response == null || $response == "" || $response == "{}"){
            Log::error('Error getting subscription status from RevenueCat API. RevenueCatController@getSubscriptionStatus');
            if($fromApi){
        ...
        */
        ...
        foreach($subscriptions as $identifier => $subs){
            ...
            if(count($orders) == 0){
                ...
                $oldSubscription = SubscriptionsModel::where([['user_id', '=', $userId], ['plan_id', '=', $planId], ['stripe_status', '=', 'active']])->first();
