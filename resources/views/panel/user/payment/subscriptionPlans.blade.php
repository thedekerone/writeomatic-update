            ...
            <div class="row row-cards">
                ...
                <div class="grid gap-3 grid-cols-4 max-lg:grid-cols-2 max-md:grid-cols-1">
                    @foreach ($plans as $plan)
                        <div
                            class="w-full border-solid border-[--tblr-border-color] rounded-3xl {{ $plan->is_featured == 1 ? 'shadow-[0_7px_20px_rgba(0,0,0,0.04)]' : '' }}">
                            <div class="flex flex-col h-full p-[1rem]">
                                <div class="flex items-start mb-2 text-[50px] leading-none text-heading font-bold">
                                    ...
                                    <div class="inline-flex flex-col items-start gap-2 mt-2 ms-2 text-[0.3em]">
                                        ...
                                        @if ($plan->is_featured == 1)
                                            <div
                                                class="inline-flex rounded-full py-[0.25rem] px-[0.75rem] bg-gradient-to-r from-[#ece7f7] via-[#e7c5e6] to-[#e7ebf9] text-[11px] text-black">
                                                {{ __('Popular plan') }}
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <ul class="list-none p-0 my-6 text-[15px] text-heading">
                                    @if ($plan->trial_days != 0)
                                        <li class="mb-3">
                                            <span
                                                class="inline-flex items-center justify-center w-[20px] h-[20px] mr-1 bg-[rgba(var(--tblr-primary-rgb),0.1)] text-primary rounded-xl align-middle">
                                    ...
                                    @if ($plan->display_word_count)
                                        <li class="mb-[0.625em]">
                                            <span
                                                class="inline-flex items-center justify-center w-[19px] h-[19px] mr-1 bg-[rgba(var(--tblr-primary-rgb),0.1)] text-primary rounded-xl align-middle">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                    viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                                                    fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                ...
                                @if ($activesubid == $plan->id)
                                    <div class="mt-auto -mx-[1rem] mb-[1rem] text-center">
                                        <div class="vstack gap-2">
                                            ...
                                            <a onclick="return confirm('Are you sure to cancel your plan? You will lose your remaining usage.');"
                                                href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.payment.cancelActiveSubscription')) }}"
                                                class="text-muted">{{ __('Cancel Subscription') }}</a>
                                        </div>
                                    </div>
                                @elseif($activesubid != null)
                                    <div class="mt-auto -mx-[1rem] mb-[1rem] text-center">
                                        <div class="vstack gap-2">
                                            <span
                                                class="text-muted"><b>{{ __('You have an active subscription.') }}</b></span>
                                        </div>
                                    </div>
                                @else
                                    <div class="mt-auto -mx-[1rem] -mb-[1rem] text-center">
                                        @if ($is_active_gateway == 1)
                                            ...
                                            <a class="btn rounded-3xl py-[0.75rem] -mx-px -mb-px w-full border border-[--tblr-border-color] text-[15px] font-semibold shadow-none hover:bg-[--tblr-primary] hover:text-white"
                                                ...
                                                data-bs-target="#gatewayModal_{{ $planid }}">{{ __('Choose plan') }}</a>
                                            <div class="modal fade" id="gatewayModal_{{ $planid }}" tabindex="-1"
                                                        ...
                                                        <div class="modal-body vstack gap-3">
                                                            @foreach ($activeGateways as $gateway)
                                                                ...
                                                                <a href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.payment.startSubscriptionProcess', ['planId' => $planid, 'gatewayCode' => $data['code']])) }}"
                                                                    ...
                                                                    <div
                                                                        class="flex justify-between w-100 align-middle items-center h-[36px] m-0 p-0">
                                                                        @if ($data['whiteLogo'] == 1)
                                                                            <img src="{{ $data['img'] }}"
                            ...
                            <div class="flex flex-col h-full p-[1rem]">
                                ...
                                <div class="mt-auto -mx-[1rem] -mb-[1rem] text-center">
                                    @if ($is_active_gateway == 1)
                                        ...
                                        <a class="btn rounded-3xl py-[0.75rem] -mx-px -mb-px w-full border border-[--tblr-border-color] text-[15px] font-semibold shadow-none hover:bg-[--tblr-primary] hover:text-white"
                                            href="#" role="button" data-bs-toggle="modal"
                                            data-bs-target="#gatewayPrepaidModal_{{ $planid }}">{{ __('Choose pack') }}</a>
                                        <div class="modal fade" id="gatewayPrepaidModal_{{ $planid }}"
                                            tabindex="-1" aria-labelledby="gatewayPrepaidModalLabel_{{ $planid }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-sm modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div class="modal-header">
