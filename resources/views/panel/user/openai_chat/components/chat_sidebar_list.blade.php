<ul class="chat-list-ul m-0 h-full list-none overflow-auto p-0 text-[14px]">
    @foreach ($list as $entry)
        ...
        >
            <a
                class="flex gap-3 p-[20px] text-[--lqd-heading-color] hover:text-[--tblr-primary] hover:no-underline group-[&.edit-mode]:pointer-events-none"
            ...
            >
                ...
                <span class="flex flex-col overflow-hidden">
                    <span class="chat-item-title max-w-[100px] group-[&.edit-mode]:pointer-events-auto">{{ __($entry->title) }}</span>
                    <span class="text-muted text-[11px]">{{ $entry->updated_at->diffForHumans() }}</span>
                </span>
            </a>
            <span class="absolute end-4 top-1/2 -translate-y-1/2">
                <button
                    class="@if ($app_is_demo && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image'))) @else  chat-item-update-title @endif btn h-[28px] w-[28px] border-[1px] border-solid border-[--tblr-border-color] bg-[--tblr-body-bg] p-0 opacity-0 group-hover:!opacity-100 group-[&.edit-mode]:!bg-[--tblr-green] group-[&.edit-mode]:!opacity-100 dark:bg-[rgba(255,255,255,0.08)] max-sm:!opacity-100"
                ...
                <button
                    class="@if ($app_is_demo && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image'))) @else chat-item-delete @endif btn bg-red border-red h-[28px] w-[28px] border-[1px] border-solid p-0 opacity-0 group-hover:!opacity-100 group-[&.edit-mode]:hidden max-sm:!opacity-100"
                    @if ($app_is_demo && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image'))) onclick="return toastr.info('This feature is disabled in Demo version.')" @endif
                >
                    <svg
                        width="10"
